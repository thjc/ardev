...
bool CLoad3DS::Import3DS(t3DModel *pModel, const char *strFileName)
{
    ...
    if(!m_FilePointer)
    {
        dbg_print(ARDBG_ERR, "Unable to find the file: %s!", strFileName);
        Quit(1);
    }
    ...
    {
        dbg_print(ARDBG_ERR, "Unable to load PRIMARY chuck from file: %s!", strFileName);
        Quit(1);
    }
...
void CLoad3DS::ProcessNextChunk(t3DModel *pModel, tChunk *pPreviousChunk)
{
    ...
    int * buffer = new int [50000];                    // This is used to read past unwanted data
	assert(buffer);
    ...
    while (pPreviousChunk->bytesRead < pPreviousChunk->length)
    {
        ...
        case OBJECTINFO:                        // This holds the version of the mesh
			...
			//printf("Process Object Info Chunk\n");
        ...
        case OBJECT:                            // This holds the name of the object being read
			...
			//printf("Process Object Chunk\n");

            // Increase the object count
            pModel->numOfObjects++;
			//printf("Loading Object Number %d\n",pModel->numOfObjects);
        ...
        case MATERIAL:                          // This holds the material information
            ...
            break;
        default:
            //printf("Skipping Unknown Chunk: %x\n",m_CurrentChunk->ID);
...
void CLoad3DS::ProcessNextMaterialChunk(t3DModel *pModel, tChunk *pPreviousChunk)
{
    ...
    while (pPreviousChunk->bytesRead < pPreviousChunk->length)
    {
        ...
        case MATNAME:                           // This chunk holds the name of the material
            //printf("got matname\n");
			if ((m_CurrentChunk->length - m_CurrentChunk->bytesRead) > 255)
			{
				printf("string we are reading is too long...%d...\n",(m_CurrentChunk->length - m_CurrentChunk->bytesRead));

			}
			//printf("Reading in mat chunk to : %p %d %d %p\n",pModel->pMaterials[pModel->numOfMaterials - 1].strName, 1, m_CurrentChunk->length - m_CurrentChunk->bytesRead, m_FilePointer);
        ...
        default:
			//printf("default mat chunk\n");
            // Read past the ignored or unknown chunks
            m_CurrentChunk->bytesRead += fread(buffer, 1, m_CurrentChunk->length - m_CurrentChunk->bytesRead, m_FilePointer);
            break;
        }
		//printf("that bit done\n");
        // Add the bytes read from the last chunk to the previous chunk passed in.
        pPreviousChunk->bytesRead += m_CurrentChunk->bytesRead;
    }
...
void CLoad3DS::ReadChunk(tChunk *pChunk)
{
	if (pChunk==NULL)
	{
		printf("Bad, pChunk was null...byeee\n");
		exit(1);
	}
